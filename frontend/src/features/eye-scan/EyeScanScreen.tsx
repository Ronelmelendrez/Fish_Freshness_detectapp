...
export default function EyeScanScreen() {
  ...
  useEffect(() => {
    ...
    const scanLine = Animated.loop(
      Animated.sequence([
        Animated.timing(scanLineAnim, { toValue: 60, duration: 1500, useNativeDriver: true }),
        Animated.timing(scanLineAnim, { toValue: -60, duration: 1500, useNativeDriver: true }),
      ])
    );
    scanLine.start();
    ...
    return (
      <View className="flex-1 bg-black">
        ...
        <View className="pb-6 px-2">
          <ScanQualityPanel response={galleryResponse} expectedPart="eye" targetSpecies={currentSpecies} />
    ...
    <View className="flex-1 bg-black">
      ...
      <View className="absolute inset-0 bg-black/20 pointer-events-none">
        ...
        <View className="flex-1 justify-center items-center">
          <Animated.View className="items-center justify-center" style={{ transform: [{ scale: scanState === "scanning" ? pulseAnim : 1 }], opacity: scanState === "ready" ? 0.5 : 1 }}>
            <View className="relative w-64 h-40 items-center justify-center">
              ...
              <View className="w-24 h-24 rounded-full border-4 border-teal-400/50"><View className="w-10 h-10 rounded-full mx-auto mt-7 bg-teal-400/30" /></View>
              {scanState === "scanning" && <Animated.View className="absolute left-16 right-16 h-0.5 bg-teal-400" style={{ transform: [{ translateY: scanLineAnim }] }} />}
            </View>
          </Animated.View>
        ...
        <View className="items-center py-4 pointer-events-auto">
          ...
          {scanState === "scanning" && (
            <>
              <ScanQualityPanel response={detectionResponse} expectedPart="eye" targetSpecies={currentSpecies} />
              <View className="flex-row justify-center gap-3 px-4 mt-2">
