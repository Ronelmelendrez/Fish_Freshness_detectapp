...
export default function SkinScanScreen() {
  ...
  const [guidance, setGuidance] = useState("Position the fish skin in the center");
      ...
      <CameraView ref={cameraRef} style={{ flex: 1 }} facing="back">
        <View className="flex-1 bg-black/30">
          ...
          <View className="flex-1 justify-center items-center">
            <View className="absolute top-1/4 left-4 w-10 h-10 border-t-2 border-l-2 border-teal-500" />
            <View className="absolute top-1/4 right-4 w-10 h-10 border-t-2 border-r-2 border-teal-500" />
            <View className="absolute bottom-1/4 left-4 w-10 h-10 border-b-2 border-l-2 border-teal-500" />
            <View className="absolute bottom-1/4 right-4 w-10 h-10 border-b-2 border-r-2 border-teal-500" />
          </View>
          ...
          <View className="flex-row justify-center items-center p-6 gap-4">
            {showManualButton && (
              <TouchableOpacity className="flex-row bg-teal-600 px-6 py-3 rounded-xl items-center gap-2" onPress={handleCapture} disabled={isProcessing}>
                <Feather name="camera" size={24} color="#fff" />
                <Text className="text-white text-base font-semibold">Capture</Text>
              </TouchableOpacity>
            )}
      ...
      {isProcessing && (
        <View className="absolute inset-0 bg-black/70 justify-center items-center">
          <View className="bg-white p-8 rounded-2xl items-center">
            <Text className="text-lg font-semibold text-slate-900">Processing...</Text>
          </View>
        </View>
      )}
    </View>
